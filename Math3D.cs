...
namespace _DCubeNoGimbalLock
{
    class Math3D
    {
        ...
        {
            public float x;
            public float y;      // Değişkenler
            public float z;

            public Vector3D(int _x, int _y, int _z) // 1.Kurucu Metod
            { 
                x = _x;
                y = _y;
                z = _z;
            ...
            public Vector3D(double _x, double _y, double _z) // 2.Kurucu Metod
            {
                x = (float)_x;
                y = (float)_y;
                z = (float)_z;
            ...
            public Vector3D(float _x, float _y, float _z) // 3.Kurucu Metod
            {
                x = _x;
                y = _y;
                z = _z;
            ...
            public Vector3D() //  4.Kurucu Metod
            {
            }

            public override string ToString()
            {
                return "(" + x.ToString() + ", " + y.ToString() + ", " + z.ToString() + ")";  // x,y,z değerlerini döndürmek için kullanılır.
            }
        }// CLASS Vector3D THE END

        internal class Camera
        {
            public Vector3D position = new Vector3D(); // Vector3D sınıfından position nesnesini (obje) yaratıyoruz.

        }//class Camera THE END

        public class Cube
        {
            // 3D ve 2D 4 NOKTA ARAYÜZÜ
            internal class Face : IComparable<Face>
            {
                public enum Side
                {
                    ...
                    Back,  // arka
                    Left,  //sol
                    Right, //sağ           //enum sabitleri belirledik
                    Top,   //üst
                    Bottom  //alt
                }
                ...
                public Vector3D Center;                // (obje) nesneler yaratıldı.
                public Side CubeSide;

                public Face() // Kurucu metod
                {
                }

                public int CompareTo(Face otherFace)
                {
                    return (int)(this.Center.z - otherFace.Center.z); // Ekranın en yakın sırası
                }
            }

            public int width = 0; //genişilik
            public int height = 0; // yükseklik
            public int depth = 0;  //derinlik
            ...
            public float RotateX
            {
                ...
                set
                {
                    //  RotateX için bu döndürme ile son dönüş arasındaki farkı döndür
            ...
            public float RotateY
            {
                ...
                set
                {
                    //  RotateY için bu döndürme ile son dönüş arasındaki farkı döndür
            ...
            public float RotateZ
            {
                ...
                set
                {
                    //  RotateZ için bu döndürme ile son dönüş arasındaki farkı döndür
                    RotateCubeZ(value - zRotation);
                    zRotation = value;
                }
            }

            public bool DrawWires    // İLK SİYAH ÇİZGİLERİ ÇİZMEK İÇİN METODUMUZ
            {
            ...
            {
                // Küpü renklerlr doldurmak için

                faces = new Face[6];  // küpün 6 ​​yüzü var
           ...
           // Her Yüz İçin 2D BOYUTUNUN HESAPLANMASI
            private void Update2DPoints(Point drawOrigin)
            {
                 //  2D noktalarını güncelleme
            ...
            // Döndürme yöntemleri, küpü döndürme noktasına çevirir (ortalar), döndürür ve geri çevrilir
            private void RotateCubeX(float deltaX)
            {
                for (int i = 0; i < faces.Length; i++)
                {
                    //DÖNDÜRME İŞLEMİNİ UYGULAMAK
                    //------DÖNDÜRME NOKTALARI
                    Vector3D point0 = new Vector3D(0, 0, 0);
                    faces[i].Corners3D = Math3D.Translate(faces[i].Corners3D, cubeOrigin, point0); //Köşeyi hareket ettirme.
                    faces[i].Corners3D = Math3D.RotateX(faces[i].Corners3D, deltaX);
                    faces[i].Corners3D = Math3D.Translate(faces[i].Corners3D, point0, cubeOrigin); //Geriye Hareketi sağlar
            ...
            public Bitmap DrawCube(Point drawOrigin)
            {
                ...
                // Son bitmap'in sınırlarını elde edin
                ...
                Array.Sort(faces); // Yüzleri SIralama
                //message();
                for (int i = faces.Length - 1; i >= 0; i--) // yüzleri arkadan öne çizmek
                {
                    switch (faces[i].CubeSide)
                    {
                        case Face.Side.Front:
                            if (fillFront)
                                g.FillPolygon(Brushes.Yellow, GetFrontFace());
                            break;
                        case Face.Side.Back:
                            if (fillBack)
                                g.FillPolygon(Brushes.Red, GetBackFace());
                            break;
                        case Face.Side.Left:
                            if (fillLeft)
                                g.FillPolygon(Brushes.Blue, GetLeftFace());
                            break;
                        case Face.Side.Right:
                            if (fillRight)
                                g.FillPolygon(Brushes.Black, GetRightFace());
                            break;
                        case Face.Side.Top:
                            if (fillTop)
                                g.FillPolygon(Brushes.White, GetTopFace());
                            break;
            ...
            // 3D noktalarını 2D noktalara dönüştürür
            ...
            {
                // Ön yüze karşılık gelen dört noktayı döndürür
                // İlgili 2D'yi al
            ...
            {
                // Doğru tarafı bulun
                // Yüzler en uzaktan en yakın sıraya göre sıralanmış olduğundan
                // Her zaman aynı dizinde olmaz
                for (int i = 0; i < faces.Length; i++)
                {
            ...
            {
                // Dönen bitmap boyutunu hesaplamak için en uzak noktaları bulun
